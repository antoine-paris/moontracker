...
const LOCATION_MARKER_DISTANCE = 0.57;
...
function EarthScene({
  ...
  const onPointerDown = (e: ThreeEvent<PointerEvent>) => {
    ...
    dragRef.current.startX = (e.nativeEvent as PointerEvent).clientX ?? 0;
    dragRef.current.startLng = selectedLng;
    ((e.nativeEvent as PointerEvent).target as HTMLElement)?.setPointerCapture?.((e.nativeEvent as PointerEvent).pointerId);
  };
  const onPointerMove = (e: ThreeEvent<PointerEvent>) => {
    if (!dragRef.current.active) return;
    const x = (e.nativeEvent as PointerEvent).clientX ?? 0;
  ...
  const endDrag = (e: ThreeEvent<PointerEvent>) => {
    ...
    ((e.nativeEvent as PointerEvent).target as HTMLElement)?.releasePointerCapture?.((e.nativeEvent as PointerEvent).pointerId);
  };
...
export default function EarthViewer3D({
  selectedLocation,
  selectedLng,
  setSelectedLng,
  utcMs,
