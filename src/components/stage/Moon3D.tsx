...
function Model({ limbAngleDeg, targetPx, modelUrl, rotOffsetDegX = 0, rotOffsetDegY = 0, rotOffsetDegZ = 0, debugMask = false, showMoonCard = false }: { limbAngleDeg: number; targetPx: number; modelUrl: string; rotOffsetDegX?: number; rotOffsetDegY?: number; rotOffsetDegZ?: number; debugMask?: boolean; showMoonCard?: boolean; }) {
  ...
  const { centeredScene, scale, radius } = useMemo(() => {
    ...
    const s = Math.max(1, targetPx) / Math.max(1e-6, maxDim);
    return { centeredScene: clone, scale: s, radius: Math.max(1e-6, maxDim) / 2 };
  }, [scene, targetPx]);
  // Orientation absolue appliquée au group parent (axesHelper suit la même rotation)
  const baseX = 0, baseY = -90, baseZ = 180;
  ...
  const northLocal = useMemo(() => new THREE.Vector3(0, 0, 1), []); // Nord lunaire = +Z local
  const viewLocal = useMemo(() => new THREE.Vector3(0, 0, -1), []); // caméra regarde -Z local
  ...
  const qEquator = useMemo(() => {
    const q = new THREE.Quaternion();
    q.setFromUnitVectors(new THREE.Vector3(0, 0, 1), northLocal); // anneau par défaut normal +Z → Nord
    return q;
  }, [northLocal]);
  const qMeridian = useMemo(() => {
    const q = new THREE.Quaternion();
    q.setFromUnitVectors(new THREE.Vector3(0, 0, 1), meridianNormalLocal); // normal du plan méridien
    return q;
  }, [meridianNormalLocal]);
       ...
       {showMoonCard && (
         <group>
           ...
           <group quaternion={qEquatorTorus} renderOrder={10}>
             <mesh renderOrder={10}>
               <torusGeometry args={[radius * 1.04, radius * 0.02, 16, 128]} />
           ...
           <group renderOrder={11}>
             <mesh position={[axisPos.x, axisPos.y, axisPos.z]} quaternion={qAxisDisk} renderOrder={11}>
               <cylinderGeometry args={[radius * 0.02, radius * 0.02, axisLen, 16]} />
...
export default function Moon3D({ x, y, wPx, hPx, moonAltDeg, moonAzDeg, sunAltDeg, sunAzDeg, limbAngleDeg, modelUrl = '/src/assets/nasa-gov-4720.glb', debugMask = false, rotOffsetDegX = 0, rotOffsetDegY = 0, rotOffsetDegZ = 0, camRotDegX = 0, camRotDegY = 0, camRotDegZ = 0, showPhase = true, showMoonCard = false }: Props) {
  ...
  const left = Math.round(x - wPx / 2);
  const top  = Math.round(y - hPx / 2);
  const targetPx = Math.floor(Math.min(wPx, hPx) * 0.9);

  return (
    <div className="absolute pointer-events-none" style={{ left, top, width: Math.max(1, wPx), height: Math.max(1, hPx), zIndex: Z.ui - 1, overflow: 'hidden' }}>
      <Canvas orthographic dpr={[1, 2]} gl={{ alpha: true }} onCreated={({ gl }) => {
        ...
        <Suspense fallback={<mesh><sphereGeometry args={[Math.min(wPx, hPx) * 0.45, 32, 32]} /><meshStandardMaterial color="#b0b0b0" /></mesh>}>
